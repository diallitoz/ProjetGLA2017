<ul class="nav nav-pills nav-stacked " style="width:170px; ">
  <li role="presentation" class="active"><a href="Gestionnaire.jsp">Menu Gestionnaire</a></li>
  
<!--  partie de gestionnaire -->
  
		<li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><font color="white">Gestion Client</font> <span class="caret"></span></a>
          <ul class="dropdown-menu">
          
              <li role="separator" class="divider"></li>
             	<!-- <li><a href="#" style="color:red">Gestion Client</a></li> -->
	             <li><a href="CreerNouveauClient.jsp">Nouveau Client</a></li>
	             <li><a href="VueClient.jsp">Operations Client</a></li>
	             <li><a href="RechercherClient.jsp">Rechercher Client</a></li>
	     
	          
	             
          </ul>
        </li>
        
        
        <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><font color="white">Gestion Compte</font> <span class="caret"></span></a>
          <ul class="dropdown-menu">

	             
	         <li role="separator" class="divider"></li>
             	<!-- <li><a href="#" style="color:red">Gestion Compte</a></li> -->
           <!--   <li role="separator" class="divider"></li> -->
	             <li><a href="CreerNouveauCompte.jsp">Nouveau Compte</a></li>
	             <li><a href="VueCompte.jsp">Operations Compte</a></li>
	             <li><a href="RechercherCompte.jsp">Rechercher Compte</a></li>
	          
	             
          </ul>
        </li>
	         
  
   <!-- ----------------------------------------------------------------------------------------------- -->
  <!--  partie de Personnaliser -->
  <li role="presentation"><a href="#"><font color="white">Personnaliser</font></a></li>
</ul>



<!-- <ul>
<li><a href="">Item 1</a></li>
<li><a href="">Item 2</a>
  <ul>
   <li><a href="">Item 2.1</a></li>
   <li><a href="">Item 2.2</a></li>
  </ul>
</li>
<li><a href="">Item 3</a></li>
</ul>
 -->



